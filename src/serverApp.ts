...
export async function serverApp(
  ...
  if (commandOptions.debug) debug.enabled("vitepress-export-pdf:*");
  ...
  if (userConfigPath) userConfig = await loadModule<UserConfig>(userConfigPath);

  // set default routePatterns
  if (Array.isArray(userConfig.routePatterns))
    userConfig.routePatterns = [
      "/**",
      "!/404.html",
      ...userConfig.routePatterns,
    ];
  else userConfig.routePatterns = ["/**", "!/404.html"];

  const vitepressOutFile =
    commandOptions.outFile ?? `vitepress-${timeTransformer()}.pdf`;
